...
# --- Deterministic ingestion tests for QIES ingestion ---


def test_qies_ingestion_is_deterministic(tmp_path):
...
def test_qies_ingestion_metadata(tmp_path):
    """QIES ingestion must attach deterministic metadata."""
    csv_path = tmp_path / "qies.csv"
    df = pd.DataFrame({col: ["x"] for col in QIES_MIN_COLUMNS})
    df.to_csv(csv_path, index=False)
    ...
    ), "QIES ingestion must attach ingestion_metadata"
...
def test_qies_ingestion_minimal_structure_is_deterministic(tmp_path):
...
def test_qies_ingestion_rejects_missing_columns(tmp_path):
    """QIES ingestion must reject CSVs missing required columns."""
    csv_path = tmp_path / "qies_bad.csv"
